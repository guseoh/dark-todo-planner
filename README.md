...
현재 저장소에는 Prisma migration 폴더가 없으므로 로컬과 단일 서버 운영에서는 `db:push`로 schema를 반영합니다. migration 파일을 생성해 운영 배포 전략을 갖춘 뒤에는 `db:deploy`를 사용하세요.
...
- `db:push`: migration 없이 현재 schema를 DB에 반영
...
개발 CORS는 `CLIENT_URL` 또는 `CLIENT_ORIGIN`에 지정한 origin만 허용합니다. 여러 origin이 필요하면 쉼표로 구분합니다. 운영 모드에서는 Express가 정적 프론트와 API를 같은 origin에서 제공하는 구성을 기본으로 봅니다.
...
기본 `npm run start`는 localhost에만 바인딩합니다. Docker Compose는 컨테이너 외부 노출을 위해 `HOST=0.0.0.0`을 설정하므로, 공개 네트워크에 배치할 때는 reverse proxy 인증이나 방화벽으로 접근 범위를 제한하세요.
