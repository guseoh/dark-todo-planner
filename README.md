...
- Node.js 22.12.0 이상, npm 10 이상
- 로컬 버전 고정이 필요하면 `.nvmrc`의 Node 22를 사용하세요.
- Docker 실행 시 Docker Desktop 또는 Docker Engine과 Docker Compose
- SQLite DB는 기본적으로 `data/*.db`에 저장됩니다. 기존 데이터를 보존하려면 `data` 폴더를 삭제하지 마세요.

### 최초 로컬 설치

```bash
git clone https://github.com/guseoh/dark-todo-planner.git
cd dark-todo-planner
npm ci
npm run db:generate
npm run db:deploy
```

저장소에는 초기 Prisma migration이 포함되어 있습니다. 새 환경과 운영 배포에서는 `db:deploy`로 커밋된 migration만 적용하세요. `db:push`는 로컬 개발 중 schema를 빠르게 맞출 때만 사용합니다.
...
기본 실행은 `127.0.0.1`에만 바인딩합니다. 다른 장비나 reverse proxy에서 접근해야 하는 운영 환경에서는 `HOST=0.0.0.0`처럼 명시적으로 설정하세요.
...
`preview`는 Vite 정적 미리보기이며 Express API와 SQLite 저장 기능을 포함하지 않습니다.
...
npm run db:check
npm run db:backup
npm run db:restore -- backups/todo-planner-YYYY-MM-DDTHH-MM-SSZ.db --confirm
npm run db:studio
```

- `db:generate`: Prisma Client 생성
- `db:push`: migration 없이 현재 schema를 DB에 반영. 운영 시작 스크립트에서는 사용하지 않습니다.
- `db:migrate`: 개발 환경에서 migration 생성 및 적용
- `db:deploy`: 이미 생성된 migration을 운영 DB에 적용
- `db:baseline`: 기존 `db:push` 기반 DB를 초기 migration이 이미 적용된 상태로 표시. 백업 후 1회만 실행하세요.
- `db:check`: SQLite `integrity_check`와 `foreign_key_check` 실행
- `db:backup`: `backups/*.db`에 SQLite 온라인 백업 생성
- `db:restore`: `backups/*.db`에서 DB 복구. `--confirm`이 필요하며 복구 전 현재 DB를 안전 백업합니다.
...
GitHub Pages는 정적 프론트만 제공하므로 Express API와 SQLite 저장 기능은 동작하지 않습니다.
...
```bash
cp .env.example .env
# .env에서 APP_AUTH_PASSWORD를 긴 임의 문자열로 설정하세요.
...
또한 `./backups:/app/backups` 볼륨을 연결해 `npm run db:backup` 결과를 호스트에 보존합니다.
...
JSON_BODY_LIMIT="2mb"
BACKUP_JSON_BODY_LIMIT="4mb"
APP_AUTH_ENABLED=false
APP_AUTH_USERNAME="todo"
APP_AUTH_PASSWORD=""
ALLOW_INSECURE_PUBLIC_ACCESS=false
```

Docker / 운영 예시:

```env
DOCKER_DATABASE_URL="file:../data/prod.db"
PORT=3000
HOST=0.0.0.0
NODE_ENV=production
...
Prisma SQLite 경로는 `prisma/schema.prisma` 기준 상대 경로입니다. `file:../data/prod.db`는 프로젝트 루트의 `data/prod.db`를 가리킵니다. Docker Compose에서는 로컬 개발용 `DATABASE_URL`과 충돌하지 않도록 `DOCKER_DATABASE_URL`을 컨테이너의 `DATABASE_URL`로 전달합니다.

`CLIENT_URL` 또는 `CLIENT_ORIGIN`에 지정한 origin만 CORS와 mutating request Origin 검사에서 허용합니다. 여러 origin이 필요하면 쉼표로 구분합니다. 운영 모드에서는 Express가 정적 프론트와 API를 같은 origin에서 제공하는 구성을 기본으로 보며, 같은 Host의 Origin도 허용합니다.
...
운영 모드에서는 `APP_AUTH_ENABLED=true`와 `APP_AUTH_USERNAME`, `APP_AUTH_PASSWORD`를 설정해야 서버가 시작됩니다. 이 앱 접근 인증은 `/api/health`를 제외한 API와 정적 프론트 모두를 HTTP Basic Auth로 보호합니다. 정말 인증 없는 공개 접근을 의도한 경우에만 `ALLOW_INSECURE_PUBLIC_ACCESS=true`를 명시하세요.

기본 `npm run start`는 localhost에만 바인딩합니다. Docker Compose는 컨테이너 외부 노출을 위해 `HOST=0.0.0.0`을 설정하고, `APP_AUTH_PASSWORD`가 비어 있으면 시작하지 않습니다. 공개 네트워크에서는 앱 접근 인증과 함께 VPN, 방화벽, 사설 네트워크, reverse proxy 접근 제한을 같이 사용하는 것을 권장합니다.
...
- 기본 저장소: SQLite
- DB 파일 위치: `data/*.db`
- SQLite DB 백업: `npm run db:backup`
- SQLite DB 무결성 검사: `npm run db:check`
- SQLite DB 복구: `npm run db:restore -- backups/<backup-file>.db --confirm`
...
`db:backup`은 SQLite `VACUUM INTO`를 사용해 `backups/*.db` 파일을 만들고, 생성된 백업에 대해 `integrity_check`와 `foreign_key_check`를 실행합니다. `db:restore`는 `backups` 폴더 안의 `.db` 파일만 허용하고, 복구 전 현재 DB를 `pre-restore-*.db`로 안전 백업합니다. 복구 작업 중에는 앱 프로세스를 중지해 새 쓰기 작업이 들어오지 않게 하세요.

이 저장소에 migration이 추가되기 전에 `db:push`로 만든 기존 DB를 운영에서 계속 쓰는 경우, 먼저 `npm run db:backup`으로 백업하고 `npm run db:baseline`을 한 번 실행한 뒤 `npm run db:deploy`로 전환하세요. 새 DB는 baseline 없이 `db:deploy`만 실행하면 됩니다.
...
- 아이콘 입력칸에는 emoji, `lucide:BookOpen`, `http/https` 이미지 URL, PNG/JPEG/GIF/WebP/AVIF `data:image` 값을 붙여넣을 수 있습니다.
- `javascript:` 같은 위험한 URL 스킴은 저장하지 않습니다.
- SVG data URL은 스크립트와 외부 리소스 위험을 줄이기 위해 허용하지 않습니다.
...
- URL / 붙여넣기 탭에서 이미지 URL이나 PNG/JPEG/GIF/WebP/AVIF `data:image` 값을 붙여넣습니다.
...
- `npm run client:dev`: Vite 프론트엔드 dev server만 실행
- `npm run server:dev`: Express dev server만 실행
- `npm run build`: React build + Express TypeScript build
- `npm run build:pages`: GitHub Pages용 `/dark-todo-planner/` base 정적 build
- `npm run test`: Node test runner + tsx로 보안/검증/DB 운영 테스트 실행
- `npm run test:ci`: CI용 테스트 명령
- `npm run start`: production Express 서버 실행
- `npm run preview`: Vite 정적 preview 실행. API 서버는 포함하지 않음
- `npm run db:generate`: Prisma Client 생성
- `npm run db:push`: Prisma schema를 DB에 반영. 로컬 개발 편의용
- `npm run db:migrate`: 개발 환경에서 Prisma migration 생성 및 적용
- `npm run db:deploy`: 운영 환경에서 이미 생성된 migration 적용
- `npm run db:baseline`: 기존 `db:push` DB를 초기 migration 적용 완료 상태로 표시
- `npm run db:check`: SQLite DB 무결성 검사
- `npm run db:backup`: `backups` 폴더에 SQLite DB 백업 생성
- `npm run db:restore -- backups/<file>.db --confirm`: 백업 DB 복구
