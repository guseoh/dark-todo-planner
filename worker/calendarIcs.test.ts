...
describe("buildCalendarIcs", () => {
  it("exports linked time blocks as timed events and avoids duplicate all-day Todo events", () => {
    ...
    expect(unfolded).toContain("UID:time-block-block-1@dark-todo-planner");
    expect(unfolded).toContain("DTSTART;TZID=Asia/Seoul:20260818T093000");
    expect(unfolded).toContain("DTEND;TZID=Asia/Seoul:20260818T110000");
    expect(unfolded).not.toContain("UID:todo-todo-linked@dark-todo-planner");
    expect(unfolded).toContain("UID:todo-todo-day@dark-todo-planner");
    expect(unfolded).toContain("DTSTART;VALUE=DATE:20260819");
    expect(unfolded).toContain("DTEND;VALUE=DATE:20260820");
    expect(unfolded).toContain("DESCRIPTION:1차 확인\\n2차 확인\\n프로젝트: 개인\\n카테고리: 정리");
    expect(ics.endsWith("\r\n")).toBe(true);
  });
  ...
  it("escapes text and folds physical lines to at most 75 UTF-8 bytes", () => {
    ...
    expect(unfolded).toContain("SUMMARY:백엔드\\, 캘린더\\; 내보내기");
