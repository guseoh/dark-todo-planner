...
describe("backup format v8", () => {
  it("keeps v7 backups compatible while exposing the new v8 collections", () => {
    const { data, warnings } = normalizeBackupPayload({
      version: 7,
      projects: [{ id: "project-1", name: "기존 프로젝트" }],
      todos: [{ id: "todo-1", title: "기존 Todo", date: "2026-08-16" }],
      memos: [{ id: "memo-1", content: "기존 메모" }],
    });

    expect(BACKUP_VERSION).toBe(8);
    expect(SUPPORTED_BACKUP_VERSIONS).toContain(7);
    expect(SUPPORTED_BACKUP_VERSIONS).toContain(8);
  ...
  it("restores only valid project decisions and memo relations", () => {
    const { data } = normalizeBackupPayload({
      ...
      memoTodoLinks: [
        { memoId: "memo-1", todoId: "todo-1", createdAt: "2026-08-16T02:00:00.000Z" },
        { memoId: "memo-1", todoId: "todo-1" },
        { memoId: "memo-1", todoId: "missing" },
      ],
      memoProjectLinks: [
        { memoId: "memo-1", projectId: "project-1" },
        { memoId: "missing", projectId: "project-1" },
      ],
    });

    const normalized = normalizeBackupV8Relations(
      data,
      {
        projectIds: new Set(["project-1"]),
        todoIds: new Set(["todo-1"]),
        memoIds: new Set(["memo-1"]),
      },
    ...
    expect(normalized.memoTodoLinks).toEqual([
      { memoId: "memo-1", todoId: "todo-1", createdAt: "2026-08-16T02:00:00.000Z" },
    ]);
    expect(normalized.memoProjectLinks).toEqual([
      { memoId: "memo-1", projectId: "project-1", createdAt: "2026-08-16T03:00:00.000Z" },
    ]);
