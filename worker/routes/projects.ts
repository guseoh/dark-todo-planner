...
import { milestoneInputSchema, projectInputSchema } from "../validation";
...
projectRoutes.get("/projects", async (c) => {
  ...
  const page = pagination((name) => c.req.query(name));
  const archived = c.req.query("archived");
  const filters = [eq(projects.userId, c.get("userId"))];
  if (archived === "true" || archived === "false") filters.push(eq(projects.archived, archived === "true"));
  else if (archived !== "all") filters.push(eq(projects.archived, false));
...
projectRoutes.post("/projects", async (c) => {
  ...
  const [maximum] = await db.select({ value: max(projects.order) }).from(projects).where(eq(projects.userId, userId));
  const now = nowIso();
  const row = {
    ...
    archived: input.archived || false, archivedAt: input.archived ? now : null, order: input.order ?? (maximum.value ?? -1) + 1,
...
projectRoutes.put("/projects/:id", async (c) => {
  ...
  await db.update(projects).set({
    name: input.name, description: optional(input.description), status: input.status, color: input.color || existing.color,
    icon: normalizeIcon(input.icon), startDate: optional(input.startDate), targetDate: optional(input.targetDate), archived,
    archivedAt: archived && !existing.archived ? nowIso() : !archived ? null : existing.archivedAt,
...
for (const action of ["archive", "unarchive"] as const) {
  projectRoutes.patch(`/projects/:id/${action}`, async (c) => {
    ...
    await db.update(projects).set({ archived, archivedAt: archived ? nowIso() : null, updatedAt: nowIso() }).where(eq(projects.id, id));
    const [row] = await db.select().from(projects).where(eq(projects.id, id));
    return c.json({ project: serialize(row) });
  });
}
...
projectRoutes.get("/milestones", async (c) => {
  ...
  const filters = [eq(milestones.userId, c.get("userId"))];
...
projectRoutes.post("/milestones", async (c) => {
  const input = milestoneInputSchema.parse(await c.req.json());
  const db = drizzle(c.env.DB);
  const userId = c.get("userId");
  ...
  const [maximum] = await db.select({ value: max(milestones.order) }).from(milestones).where(eq(milestones.projectId, input.projectId));
  const now = nowIso();
  const row = {
    id: newId(), userId, projectId: input.projectId, title: input.title, description: optional(input.description),
    targetDate: optional(input.targetDate), status: input.status, order: input.order ?? (maximum.value ?? -1) + 1, createdAt: now, updatedAt: now,
  };
  await db.insert(milestones).values(row);
  return c.json({ milestone: serialize(row) }, 201);
});
...
projectRoutes.put("/milestones/:id", async (c) => {
  ...
  const db = drizzle(c.env.DB);
  const id = c.req.param("id"), userId = c.get("userId");
  const [existing] = await db.select().from(milestones).where(and(eq(milestones.id, id), eq(milestones.userId, userId))).limit(1);
  if (!existing) return c.json({ message: "마일스톤을 찾을 수 없습니다." }, 404);
  const [project] = await db.select({ id: projects.id }).from(projects).where(and(eq(projects.id, input.projectId), eq(projects.userId, userId))).limit(1);
  if (!project) return c.json({ message: "프로젝트를 찾을 수 없습니다." }, 400);
  await db.update(milestones).set({
    projectId: input.projectId, title: input.title, description: optional(input.description), targetDate: optional(input.targetDate),
    status: input.status, order: input.order ?? existing.order, updatedAt: nowIso(),
  }).where(eq(milestones.id, id));
  const [row] = await db.select().from(milestones).where(eq(milestones.id, id));
  return c.json({ milestone: serialize(row) });
});

projectRoutes.delete("/milestones/:id", async (c) => {
  const db = drizzle(c.env.DB);
  await db.delete(milestones).where(and(eq(milestones.id, c.req.param("id")), eq(milestones.userId, c.get("userId"))));
