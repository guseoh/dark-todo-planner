...
import { categories, goals, memos, milestones, musicLinks, projects, reflections, todos, topics } from "../db/schema";
...
const KEYS = ["categories", "projects", "milestones", "todos", "reflections", "goals", "memos", "topics", "topicLinks", "musicLinks"] as const;
...
async function buildBackup(env: Bindings, userId: string) {
  ...
  const [categoryRows, projectRows, milestoneRows, todoRows, reflectionRows, goalRows, memoRows, topicRows, musicRows] = await Promise.all([
  ...
  return {
    version: BACKUP_VERSION, exportedAt: nowIso(), categories: categoryRows.map(serializeCategory), projects: projectRows, milestones: milestoneRows,
    todos: await serializeTodos(db, todoRows), reflections: reflectionRows.map(serializeReflection), goals: goalRows.map(serializeGoal), memos: memoRows.map(serializeMemo),
    topics: serializedTopics, topicLinks: serializedTopics.flatMap((topic) => topic.links), musicLinks: musicRows.map(serializeMusicLink),
  };
}
...
async function importBackup(env: Bindings, userId: string, input: unknown) {
  ...
  for (const item of data.memos || []) {
    ...
    memoRows.push([String(item.id), userId, optional(item.title as string), String(item.content), optional(item.color as string), bool(item.pinned), dateValue(item.createdAt, now), dateValue(item.updatedAt, now)]); imported.memos++;
  }
  ...
  return { version: data.version ?? "legacy", latestVersion: BACKUP_VERSION, supportedVersions: [1, 2, 3, 4, 5, 6, 7], warnings, imported };
}
...
const handleImport = async (c: Context<{ Bindings: Bindings; Variables: Variables }>) => {
  ...
  catch (error) { if (error instanceof BackupError) return c.json({ message: error.message }, 400); throw error; }
};
