...
export const SUPPORTED_BACKUP_VERSIONS = [1, 2, 3, 4, 5, 6, 7, 8] as const;
...
const asItems = (value: unknown) =>
  Array.isArray(value)
    ? value.filter((item): item is BackupItem => !!item && typeof item === "object" && !Array.isArray(item))
    : [];

export const normalizeBackupPayload = (input: unknown): { data: BackupPayload; warnings: string[] } => {
  if (Array.isArray(input)) {
    return {
      data: { todos: asItems(input) },
      warnings: ["버전 없는 Todo 배열을 legacy 백업으로 처리했습니다."],
    };
  }
...
export type BackupRelationReferences = {
  projectIds: ReadonlySet<string>;
  todoIds: ReadonlySet<string>;
  memoIds: ReadonlySet<string>;
};

export type NormalizedProjectDecision = {
  id: string;
  projectId: string;
  title: string;
  decision: string;
...
export type NormalizedMemoTodoLink = {
  memoId: string;
  todoId: string;
  createdAt: string;
};

export type NormalizedMemoProjectLink = {
  memoId: string;
  projectId: string;
  createdAt: string;
};
...
export const normalizeBackupV8Relations = (data: BackupPayload, refs: BackupRelationReferences, now: string) => {
  ...
  for (const item of data.projectDecisions || []) {
    ...
    const projectId = requiredString(item.projectId);
    const title = requiredString(item.title);
    const decision = requiredString(item.decision);
    const decidedAt = requiredString(item.decidedAt);
    if (!id || decisionIds.has(id) || !projectId || !refs.projectIds.has(projectId) || !title || !decision || !decidedAt) {
      skipped.projectDecisions++;
      continue;
    }
    decisionIds.add(id);
  ...
  for (const item of data.memoTodoLinks || []) {
    const memoId = requiredString(item.memoId);
    const todoId = requiredString(item.todoId);
    const key = `${memoId}\u0000${todoId}`;
    if (!memoId || !todoId || !refs.memoIds.has(memoId) || !refs.todoIds.has(todoId) || todoLinkKeys.has(key)) {
      skipped.memoTodoLinks++;
      continue;
    }
    todoLinkKeys.add(key);
    memoTodoLinks.push({ memoId, todoId, createdAt: timestamp(item.createdAt, now) });
  }
  ...
  for (const item of data.memoProjectLinks || []) {
    const memoId = requiredString(item.memoId);
    const projectId = requiredString(item.projectId);
    const key = `${memoId}\u0000${projectId}`;
    if (!memoId || !projectId || !refs.memoIds.has(memoId) || !refs.projectIds.has(projectId) || projectLinkKeys.has(key)) {
      skipped.memoProjectLinks++;
      continue;
    }
    projectLinkKeys.add(key);
    memoProjectLinks.push({ memoId, projectId, createdAt: timestamp(item.createdAt, now) });
  }
