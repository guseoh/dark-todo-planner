...
function App({ onLogout }: { onLogout: () => Promise<void> }) {
  ...
  const content = {
    today: <TodayPage todayTodos={todayTodos} stats={planner.stats} onAdd={planner.addTodo} onToggle={planner.toggleTodo} onDelete={planner.deleteTodo} onUpdate={planner.updateTodo} categories={planner.categories} projects={planner.projects} onAddCategory={planner.addCategory} onUpdateCategory={planner.updateCategory} onDeleteCategory={planner.deleteCategory} onReorderCategories={planner.reorderCategories} overdueTodos={overdueIncompleteTodos} onBringOverdueTodos={planner.bringOverdueTodosToToday} />,
  ...
  return (
    <div className="min-h-screen pb-20 lg:pb-0">
      ...
      {showQuickAdd ? <Modal title="빠른 Todo 추가" description="Ctrl+Shift+K · 내일 · !high · #태그 · 45m · due:2026-08-20 같은 빠른 문법을 사용할 수 있습니다." onClose={() => setShowQuickAdd(false)}><TodoForm compact submitLabel="Todo 추가" categories={planner.categories} projects={planner.activeProjects} onAdd={(input) => { void planner.addTodo(input).then((created) => { if (created) setShowQuickAdd(false); }); }} /></Modal> : null}
      {planner.pendingTodoDelete || planner.pendingMemoDelete ? <div className="fixed bottom-20 right-4 z-[90] flex w-[min(24rem,calc(100vw-2rem))] flex-col gap-2 lg:bottom-4" aria-live="polite">
