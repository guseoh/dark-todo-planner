...
import { SmartViewDirectPanel } from "../components/planning/SmartViewDirectPanel";
...
export function PlanningHubPage({
  ...
  useEffect(() => {
    if (openSmartViewId) setMode("planning");
  }, [openSmartViewId]);

  const modeButtonClass = (active: boolean) =>
    `relative flex min-h-9 items-center gap-2 rounded-md px-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/40 ${active ? "bg-ink-800 text-ink-100 ring-1 ring-inset ring-ink-700/80" : "text-ink-400 hover:bg-ink-800/70 hover:text-ink-100"}`;

  return (
    <div className="mx-auto w-full max-w-[1180px] space-y-4">
      <div className="flex justify-end">
        <div className="inline-flex rounded-lg border border-ink-700/70 bg-ink-950/45 p-1">
          <button type="button" className={modeButtonClass(mode === "planning")} onClick={() => { onExitSmartView?.(); setMode("planning"); }} aria-pressed={mode === "planning"}>
            {mode === "planning" ? <span aria-hidden="true" className="absolute inset-x-3 bottom-0 h-0.5 rounded-full bg-accent-500" /> : null}
            <ListChecks size={15} />계획·리뷰
          </button>
          <button type="button" className={modeButtonClass(mode === "time")} onClick={() => { onExitSmartView?.(); setMode("time"); }} aria-pressed={mode === "time"}>
            {mode === "time" ? <span aria-hidden="true" className="absolute inset-x-3 bottom-0 h-0.5 rounded-full bg-accent-500" /> : null}
            <Clock3 size={15} />시간 계획
          </button>
        </div>
      </div>

      {mode === "planning" ? (
        openSmartViewId ? <SmartViewDirectPanel todos={planningProps.todos} smartViewId={openSmartViewId} onExit={onExitSmartView} /> : <PlanningPage {...planningProps} />
      ) : (
