...
const createResourceId = () => typeof crypto !== "undefined" && "randomUUID" in crypto
  ? crypto.randomUUID()
...
export function ProjectPage({
  ...
  const deleteSelectedProject = async () => {
    if (!selected || deletingProject) return;
    const confirmed = window.confirm(
      `"${selected.name}" 프로젝트를 삭제할까요?\n\n연결된 Todo는 유지되지만 프로젝트 연결이 해제됩니다. 마일스톤과 의사결정 기록은 함께 삭제되며 되돌릴 수 없습니다.`,
    );
    if (!confirmed) return;

    setDeletingProject(true);
    try {
      const deleted = await Promise.resolve(onDeleteProject(selected.id));
  ...
  const addResource = async (event: FormEvent) => {
    ...
    if (!label) {
      setResourceError("링크 이름을 입력하세요.");
      return;
    }
    if (!isHttpUrl(url)) {
      setResourceError("http 또는 https 주소를 입력하세요.");
      return;
    }

    const resources = selected.resources || [];
    if (resources.length >= 12) {
      setResourceError("프로젝트 자료 링크는 최대 12개까지 등록할 수 있습니다.");
      return;
    }
    if (resources.some((resource) => resource.url === url)) {
      setResourceError("이미 등록된 주소입니다.");
      return;
    }
  ...
  const removeResource = async (resourceId: string) => {
    ...
    await onUpdateProject(selected.id, {
      resources: (selected.resources || []).filter((resource) => resource.id !== resourceId),
    });
  };
  ...
  const createMilestone = async (event: FormEvent) => {
    ...
    const created = await onAddMilestone({
    ...
    <div className="space-y-5">
      <section>
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-2xl font-bold text-ink-100 sm:text-3xl">프로젝트</h2>
            <p className="mt-2 text-sm text-ink-400">장기 작업을 마일스톤·Todo·의사결정 기록과 함께 관리합니다.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            {!showArchived ? (
              <button type="button" className="btn-primary" onClick={() => setShowCreateProject((value) => !value)}>
                <Plus size={16} />{showCreateProject ? "추가 닫기" : "프로젝트 추가"}
              </button>
            ) : null}
            <button type="button" className="btn-secondary" onClick={() => { setShowArchived((value) => !value); setShowCreateProject(false); }}>
              {showArchived ? <ArchiveRestore size={16} /> : <Archive size={16} />}
              {showArchived ? "진행 프로젝트" : `보관함 ${archivedProjects.length}`}
            </button>
          </div>
        </div>
      </section>

      {showCreateProject && !showArchived ? (
        <form className="app-card grid gap-2 p-3 sm:grid-cols-[minmax(0,1fr)_11rem_auto]" onSubmit={createProject}>
          <input className="field" value={newName} onChange={(event) => setNewName(event.target.value)} placeholder="새 프로젝트 이름" autoFocus />
          ...
          <button className="btn-primary" type="submit" disabled={!newName.trim()}><Plus size={16} />추가</button>
        </form>
      ) : null}

      <section className="app-card p-2" aria-label={showArchived ? "보관 프로젝트 선택" : "진행 프로젝트 선택"}>
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-0.5">
            {visibleProjects.map((project) => (
              <button
                ...
                className={`flex min-h-10 min-w-44 max-w-64 shrink-0 items-center gap-2 rounded-lg border px-3 text-left text-sm font-semibold transition ${selectedId === project.id ? "border-accent-500/35 bg-accent-500/[0.08] text-ink-100" : "border-transparent text-ink-400 hover:border-ink-700 hover:bg-ink-800/65 hover:text-ink-100"}`}
              >
        ...
        ) : <p className="px-3 py-5 text-center text-sm text-ink-500">{showArchived ? "보관된 프로젝트가 없습니다." : "프로젝트를 하나 만들어보세요."}</p>}
      </section>
      ...
      {selected ? (
        <div className="min-w-0 space-y-4">
          <section className="app-card p-4 sm:p-5">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <FolderKanban size={20} className="text-accent-300" />
                ...
                <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-xs text-ink-400">
                  ...
                  <span>마일스톤 {projectMilestones.length}개</span>
                  <span>자료 {(selected.resources || []).length}개</span>
                  <span>결정 {projectDecisions.length}개</span>
                </div>
              </div>

              <div className="flex flex-wrap gap-2">
                {!selected.archived ? <button type="button" className="btn-secondary" onClick={beginProjectEdit}><Pencil size={15} />정보 수정</button> : null}
                <select className="field min-h-10 w-auto py-1.5 text-sm" value={selected.status} onChange={(event) => void onUpdateProject(selected.id, { status: event.target.value as ProjectStatus })} disabled={selected.archived || deletingProject}>
                  <option value="PLANNING">계획</option>
                  <option value="ACTIVE">진행 중</option>
                  <option value="ON_HOLD">보류</option>
                  <option value="DONE">완료</option>
                </select>
                <button type="button" className="btn-secondary hover:border-danger/50 hover:text-red-200" onClick={() => void deleteSelectedProject()} disabled={deletingProject}>
                  <Trash2 size={15} />{deletingProject ? "삭제 중" : "삭제"}
                </button>
                {selected.archived ? (
                  <button type="button" className="btn-secondary" onClick={() => void onUnarchiveProject(selected.id)} disabled={deletingProject}><ArchiveRestore size={15} />복원</button>
                ) : (
                  <button type="button" className="btn-secondary" onClick={() => void onArchiveProject(selected.id)} disabled={deletingProject}><Archive size={15} />보관</button>
                )}
              </div>
            </div>
            ...
            {editingProject ? (
              <form className="mt-4 grid gap-3 rounded-lg border border-ink-700/70 bg-ink-950/25 p-3 lg:grid-cols-2" onSubmit={saveProjectInfo}>
                ...
                <div className="flex justify-end gap-2 lg:col-span-2">
                  <button type="button" className="btn-secondary" onClick={() => setEditingProject(false)}><X size={15} />취소</button>
                  <button type="submit" className="btn-primary" disabled={!projectName.trim()}><Save size={15} />저장</button>
                </div>
              </form>
            ) : null}
            ...
            <div className="mt-4 border-t border-ink-800/80 pt-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {(selected.resources || []).map((resource) => (
                    <div key={resource.id} className="inline-flex max-w-full items-center rounded-md border border-ink-700/70 bg-ink-950/35">
                      <a className="inline-flex min-h-9 min-w-0 items-center gap-1.5 px-2.5 text-xs font-semibold text-ink-300 hover:text-accent-200" href={resource.url} target="_blank" rel="noreferrer" title={resource.url}>
              ...
              ) : <p className="mt-3 text-xs text-ink-500">자주 여는 저장소, 배포 주소, 문서를 연결해두면 프로젝트 화면에서 바로 이동할 수 있습니다.</p>}

              {!selected.archived ? (
                <form className="mt-3 grid gap-2 lg:grid-cols-[11rem_minmax(0,1fr)_auto]" onSubmit={addResource}>
                  <input className="field min-h-9 py-1.5 text-xs" value={resourceLabel} onChange={(event) => setResourceLabel(event.target.value)} placeholder="예: GitHub 저장소" maxLength={80} />
                ...
                return (
                  <div key={milestone.id} className="rounded-lg border border-ink-800/70 bg-ink-950/25 p-3">
                    <div className="flex items-start gap-2">
                      <button type="button" className="icon-btn h-9 w-9 shrink-0 rounded-md" onClick={() => void onUpdateMilestone(milestone.id, { status: milestone.status === "DONE" ? "TODO" : "DONE" })} aria-label="마일스톤 완료 토글">
                        {milestone.status === "DONE" ? <CheckCircle2 size={16} className="text-emerald-300" /> : <CircleDot size={16} />}
                      </button>
                    ...
                    <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-ink-800">
                      <div className="h-full rounded-full bg-accent-500 transition-all" style={{ width: `${milestoneProgress}%` }} />
                    </div>
        ...
        <div className="app-card flex min-h-56 items-center justify-center p-6 text-center">
          <div>
            <PauseCircle className="mx-auto text-ink-600" />
            <p className="mt-3 text-sm font-semibold text-ink-400">프로젝트를 선택하거나 새로 만들어보세요.</p>
          </div>
        </div>
      )}
    </div>
