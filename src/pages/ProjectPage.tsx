...
import { Archive, ArchiveRestore, CheckCircle2, CircleDot, ExternalLink, FileText, FolderKanban, Link2, PauseCircle, Pencil, Plus, Save, Target, Trash2, X } from "lucide-react";
...
const memoTitle = (memo: Memo) => memo.title
  || memo.content.split("\n").find((line) => line.trim())?.replace(/^[-#>*\s]+/, "").slice(0, 36)
  || "제목 없음";
    ...
    <div className="space-y-5">
      <section>
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-ink-400">장기 작업을 마일스톤·하위 작업·의사결정 기록과 함께 관리합니다.</p>
          </div>
          <button type="button" className="btn-secondary" onClick={() => setShowArchived((value) => !value)}>
      ...
      <div className="grid gap-5 xl:grid-cols-[18rem_minmax(0,1fr)]">
          ...
          <div className="app-card space-y-1 p-2">
            {visibleProjects.length ? visibleProjects.map((project) => (
              <button key={project.id} type="button" onClick={() => setSelectedId(project.id)} className={`relative flex min-h-11 w-full items-center gap-2 rounded-lg border px-3 text-left text-sm font-semibold transition ${selectedId === project.id ? "border-ink-700/80 bg-ink-800 text-ink-100" : "border-transparent text-ink-400 hover:border-ink-800 hover:bg-ink-800/65 hover:text-ink-100"}`}>
                ...
                <span className="text-[10px] text-ink-500">{projectStatusLabel[project.status]}</span>
              </button>
        ...
        {selected ? (
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  ...
                  <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-xs text-ink-400">
                    ...
                    <span>연결 메모 {linkedMemos.length}개</span>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <select className="field min-h-10 w-auto py-1.5 text-sm" value={selected.status} onChange={(event) => void onUpdateProject(selected.id, { status: event.target.value as ProjectStatus })} disabled={selected.archived}>
                  ...
                  {selected.archived ? (
                    <button type="button" className="btn-secondary" onClick={() => void onUnarchiveProject(selected.id)}><ArchiveRestore size={15} />복원</button>
                  ) : (
                    <button type="button" className="btn-secondary" onClick={() => void onArchiveProject(selected.id)}><Archive size={15} />보관</button>
                  )}
                </div>
              </div>
              ...
              <div className="mt-4 border-t border-ink-800/80 pt-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-ink-500">Notion · GitHub · 배포 · 문서</span>
                  </div>
                ...
                {linkedMemos.length ? (
                  <div className="space-y-2">
                    {linkedMemos.map((memo) => (
                      <div key={memo.id} className="rounded-lg border border-ink-800/70 bg-ink-950/25 px-3 py-2">
                        <p className="text-sm font-semibold text-ink-100">{memoTitle(memo)}</p>
                        <p className="mt-1 line-clamp-2 text-xs text-ink-500">{memo.content}</p>
                      </div>
                    ))}
                  </div>
                ) : <p className="py-4 text-center text-sm text-ink-500">메모 화면에서 이 프로젝트를 연결하면 여기에 표시됩니다.</p>}
              </section>
            </div>
          ...
          <div className="app-card flex min-h-64 items-center justify-center p-6 text-center">
            <div>
