  ...
  return (
    <div className="app-card p-3">
      <div className="flex flex-col gap-2 xl:flex-row xl:items-center">
        ...
        <div className="flex flex-wrap gap-1.5" aria-label="완료 상태 필터">
          {statusFilters.map((filter) => (
            <button key={filter.value} type="button" onClick={() => onChange({ ...filters, status: filter.value })} className={`${filterButtonClassName} ${buttonTone(filters.status === filter.value)}`}>
              {filter.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-1.5" aria-label="우선순위 필터">
          {priorityFilters.map((filter) => (
            <button key={filter.value} type="button" onClick={() => onChange({ ...filters, priority: filter.value })} className={`${filterButtonClassName} ${buttonTone(filters.priority === filter.value)}`}>
              {filter.label}
            </button>
          ))}
        </div>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        <button type="button" className={`${filterButtonClassName} ${buttonTone(filters.duplicatesOnly)}`} onClick={() => onChange({ ...filters, duplicatesOnly: !filters.duplicatesOnly })} aria-pressed={filters.duplicatesOnly}>
          중복 후보만
        </button>
        ...
        <button type="button" className="btn-secondary min-h-8 px-2.5 py-1 text-xs" onClick={() => onChange(defaultFilters)}>
      ...
      {showAdvanced ? (
        <div className="mt-2 flex flex-wrap gap-1.5 border-t border-ink-700/55 pt-2">
          ...
          <select className="field h-9 min-h-9 w-full py-1 sm:w-44" value={filters.sort} onChange={(event) => onChange({ ...filters, sort: event.target.value as TodoFilters["sort"] })} aria-label="정렬">
