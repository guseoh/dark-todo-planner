...
const newItem = (): DraftItem => ({ key: crypto.randomUUID(), title: "", priority: "MEDIUM", projectId: "", categoryId: "" });
...
export function RoutinePanel({ categories, projects, onTodosCreated }: { categories: Category[]; projects: Project[]; onTodosCreated: () => unknown | Promise<unknown> }) {
  ...
  return (
    <section className="app-card p-4 sm:p-5" aria-labelledby="routine-panel-title">
        ...
        {routines.map((routine) => {
          ...
          return (
            <article key={routine.id} className="rounded-xl border border-ink-700/70 bg-ink-950/30 p-3.5">
              ...
              <ul className="mt-3 space-y-1.5 text-xs text-ink-400">{routine.items.slice(0, 5).map((item) => <li key={item.id || item.title} className="flex items-center justify-between gap-2"><span className="truncate">• {item.title}</span><span className="shrink-0 text-ink-500">{item.priority}</span></li>)}{routine.items.length > 5 ? <li className="text-ink-500">외 {routine.items.length - 5}개</li> : null}</ul>
      ...
      {editing ? <Modal title={editing === "new" ? "루틴 만들기" : "루틴 수정"} description="항목별 프로젝트·카테고리·우선순위를 저장할 수 있습니다." onClose={() => setEditing(null)} size="lg">
        <form onSubmit={save} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {items.map((item, index) => <div key={item.key} className="grid gap-2 rounded-lg border border-ink-700/60 bg-ink-950/30 p-3 md:grid-cols-[minmax(0,1.6fr)_8rem_minmax(0,1fr)_minmax(0,1fr)_auto]">
